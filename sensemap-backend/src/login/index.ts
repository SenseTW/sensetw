...
export function router(context: Context) {
  ...
  router.get("/forget-password", bypassAuthenticated(), async (req, res) => {
    const messages = {
      emailError: req.flash("emailError")
    };
  ...
  router.post("/forget-password", bypassAuthenticated(), async (req, res) => {
    ...
    if (!user) {
      req.flash(
        "emailError",
        "Sorry, an account with this email address does not exist."
      );
      return res.redirect("/forget-password");
    }
...
<p>If you don’t want to change it, just ignore this email. Your password will stay the same. If you have more questions, send us a note at <a href="mailto:hello@sense.tw">hello@sense.tw</a>. We’d love to help!</p>
  ...
  router.get("/reset-password", bypassAuthenticated(), async (req, res) => {
  ...
  router.post("/reset-password", bypassAuthenticated(), async (req, res) => {
  ...
  router.get("/oauth/success", requireAuthentication(), async (req, res) => {
    return res.redirect(
      `/oauth/authorize?client_id=${
        hypothesisClient.id
      }&state=login&response_type=code`
    );
  });
  router.all("/oauth/revoke", requireAuthentication(), async (req, res) => {
