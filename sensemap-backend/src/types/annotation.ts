...
function toAnnotationField(args: Annotation) {
  const data = filter(x => !!x, assoc(
    'target', typeof(args.target) === 'string' ? args.target : JSON.stringify(args.target),
    pick(annotationDataFields, args)));
  return data;
}

export async function createAnnotation(db: Knex, args: any): Promise<Annotation> {
...
export async function updateAnnotation(db: Knex, id: ID, args: any): Promise<Annotation> {
