...
describe('allObjects', () => {
  test('no filter', async () => {
    const xs = await resolvers.Query.allObjects(null, {}, context({ req: null }), null);
    const x = xs[0];
  ...
  test('map filter', async () => {
    const xs0 = await resolvers.Query.allObjects(null, {}, context({ req: null }), null);
    const xs1 = await resolvers.Query.allObjects(null,
